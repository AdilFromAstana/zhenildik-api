...
import { IsString, IsNotEmpty, IsOptional, IsEnum, IsNumber, IsArray, IsInt, Min, Max, IsUrl } from 'class-validator';
...
export class CreateOfferDto {
  ...
  @ApiProperty({ required: false }) @IsOptional() @Type(() => Number) @IsNumber()
  categoryId?: number;

  @ApiProperty({ required: false }) @IsOptional() @IsString()
  cityCode?: string;

  @ApiProperty({ enum: BenefitKind }) @IsEnum(BenefitKind)
  benefitKind: BenefitKind;

  @ApiProperty({ enum: OfferScope }) @IsEnum(OfferScope)
  scope: OfferScope;
  ...
  // Привязки
  @ApiProperty({ required: false, type: [Number] }) @IsOptional() @IsArray()
  locationIds?: number[];
  ...
  @IsOptional() @IsEnum(OfferChannelCode)
  primaryChannel?: OfferChannelCode;

  @ApiProperty({ required: false })
  @IsOptional() @IsUrl({ require_tld: false }) // локальные/deeplink тоже пропустим
  ctaUrl?: string;
